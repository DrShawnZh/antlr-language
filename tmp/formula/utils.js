...
export function toggleRangeInfo(flag) {
...
export function getRangeInfo(ctx) {
...
export function getTerminalNodeInfo(terminalNode) {
...
function traverse(tree, result) {
  if (tree instanceof TerminalNodeImpl) {
    ...
    let children = [];
    let name = Object.getPrototypeOf(tree).constructor.name.replace(
      /Context$/,
      ""
    );
...
export const formulaFormat = (() => {
  const format = (ast) => {
    ...
    if (
      ast &&
      Object.prototype.toString.call(ast.formulas) === "[object Array]"
    ) {
  ...
  var formulaStrategy = {
    ...
    FormulaIfFunction: (obj, newLine, space) => {
      ...
      const thenExp = formatAtomicFormula(
        obj.thenStatement.content,
        true,
        space
      );
      const elseExp = formatAtomicFormula(
        obj.elseStatement.content,
        true,
        space
      );
    ...
    FormulaBinaryOperationExpress: (obj, newLine, space) => {
      ...
      if (obj.rightExp) {
        rawAtomicFormula += `${formatAtomicFormula(
          obj.rightExp,
          false,
          space
        )}`;
